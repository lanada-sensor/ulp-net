...
NetDeviceContainer
WifiHelper::Install (const WifiPhyHelper &phyHelper,
                     const WifiMacHelper &macHelper, NodeContainer c, AddressTable* table,
                     uint32_t isMain, uint32_t mainType, uint32_t channelType, uint32_t dataRate) const
{
	NetDeviceContainer* devices = new NetDeviceContainer;
		  if(isMain==1){
			  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
				{
				  Ptr<Node> node = *i;
				  Ptr<WifiNetDevice> device = CreateObject<WifiNetDevice> (true);
				  Ptr<WifiRemoteStationManager> manager = m_stationManager.Create<WifiRemoteStationManager> ();
				  Ptr<WifiMac> mac = macHelper.Create ();
				  Ptr<WifiPhy> phy = phyHelper.Create (node, device);
				  mac->SetAddress (Mac48Address::Allocate ());
				  ...
				  device->SetPhy (phy);
				  device->SetRemoteStationManager (manager);
				  device->SetAddressTable(table);//kdw

				  device->SetMacLow();
				  device->SetValue(1,isMain);
				  device->SetValue(2,mainType);
				  device->SetValue(3,channelType);
				  device->SetDeviceContainer(devices);
//				  if(isMain == 1) //shared chanel main device
//				  {
//					  device->MacLowInfo(2);
//				  }
//				  device->SetWifiNetDevicePtr();
				  node->AddDevice (device);
				  devices->Add (device);
				  NS_LOG_DEBUG ("node=" << node << ", mob=" << node->GetObject<MobilityModel> ());
				}
			  ...
			  uint32_t nDevice;
			  TypeId tid_main = TypeId::LookupByName("ns3::WifiNetDevice");
			  Ptr<NetDevice> main;
			  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
				{
				  Ptr<Node> node = *i;
				  Ptr<WifiNetDevice> device = CreateObject<WifiNetDevice> (false);
				  Ptr<WifiRemoteStationManager> manager = m_stationManager.Create<WifiRemoteStationManager> ();
				  Ptr<WifiMac> mac = macHelper.Create ();
				  Ptr<WifiPhy> phy = phyHelper.Create (node, device);
				  mac->SetAddress (Mac48Address::Allocate ());
...
//				  }
			      ...
			      for(uint32_t j=0; j<nDevice; j++)
			      {
			    	  main = node->GetDevice(j);
			    	  if (main->GetTypeId() == tid_main)
			    		  break;
			      }
			      Ptr<WifiNetDevice> mainTemp = DynamicCast<WifiNetDevice>(main);
			      device->SetMainDevice(mainTemp);
			      mainTemp->SetUlpDevice(device);
			      Address mainAddress(main->GetAddress());
			      Address ulpAddress(device->GetAddress());
			      table->AddAddress(mainAddress,ulpAddress);
