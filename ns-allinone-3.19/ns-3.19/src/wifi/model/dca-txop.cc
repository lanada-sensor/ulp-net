...
DcaTxop::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::DcaTxop")
    ...
    .AddTraceSource("DcaTxOpCollision","Tracing for Collision",
      		 	 	 	MakeTraceSourceAccessor (&DcaTxop::m_dcatxopCollision))
...
{
	if(this->m_low->GetValue(1) == 2) // if ULP // kdw
		m_currentPacket = 0;
...
DcaTxop::NotifyCollision (void)
{
  ...
  m_dcatxopCollision=m_dcatxopCollision+1;
  m_dcf->StartBackoffNow (m_rng->GetNext (0, m_dcf->GetCw ()));
...
//	  std::cout<<"drop1! "<<this->m_low->GetValue(1)<<std::endl; // kdw
  ...
  RestartAccessIfNeeded ();
//  if (txDone){
//	  this->m_low->GetDevice()->RetransmissionReq(1);
//  }
//  else
//  {
	  if (reTx)
		  this->m_low->GetDevice()->RetransmissionReq(0);
//  }
}
void
