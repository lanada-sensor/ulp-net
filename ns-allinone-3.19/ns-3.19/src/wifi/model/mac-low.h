...
#include "ns3/traced-value.h" // kdw
...
public:
  ...
  static TypeId GetTypeId (void); //JJH 0529 for tracing
  ...
  void UlpData();
  bool SetDevice(Ptr<WifiNetDevice> device);
  Ptr<WifiNetDevice> GetDevice();
  bool IsSleep();
  int32_t GetSleepCnt();
  void SetLowSleep(uint32_t isSleepReq, uint32_t isSender, uint32_t option);
  int32_t GetLowSleep(uint32_t option);
  void SetPacketId(uint32_t option,uint32_t packetId);
  uint32_t GetPacketId(uint32_t option);
  std::list<uint32_t> request;
  std::list<uint32_t> sleepRequest;
  ///////////////////////////////////////
...
private:
  ...
  Mac48Address mainDest;
  Ptr<WifiNetDevice> m_device;
  int32_t m_sleep_s_cnt;
  int32_t m_sleep_r_cnt;
  TracedValue<int32_t> m_sleep_cnt;
  uint32_t sendPacketId;
  uint32_t recvPacketId;
  uint32_t checkPacketId;
